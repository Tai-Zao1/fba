...
@router.post('/register', summary='注册平台用户')
...
async def add_user(request: Request, obj: AddUserParam) -> ResponseSchemaModel[GetUserInfoDetail]:
    ...
    current_user = await user_service.get_userinfo(phone=obj.phone)
...
async def get_user(phone: Annotated[str, Path(...)]) -> ResponseSchemaModel[GetUserInfoDetail]:
    current_user = await user_service.get_userinfo(phone=phone)
    data = GetUserInfoDetail(**select_as_dict(current_user))
    return response_base.success(data=data)
...
async def get_pagination_users(
    ...
    user_select = await user_service.get_select(dept=dept,
                                                username=username,
                                                phone=phone,
                                                status=status,
                                                user_id=user_id)
