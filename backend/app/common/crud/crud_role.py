...
from backend.app.common.model import DataRule, Menu, Role, User
...
class CRUDRole(CRUDPlus[Role]):
    ...
    async def get_all(self, db, store_id: int) -> Sequence[Role]:
        """
        获取所有角色
        ...
        :return:
        """
        return await self.select_models(db, store_id=store_id)
    ...
    async def get_by_name(self, db, name: str, store_id: int) -> Role | None:
        """
        通过 name 获取角色
    ...
    async def delete(self, db, role_id: list[int], store_id: int) -> int:
        """
        删除角色
        ...
        return await self.delete_model_by_column(db, allow_multiple=True, id__in=role_id, store_id=store_id)
