...
from backend.app.common.model import Dept, Role, User
...
class CRUDUser(CRUDPlus[User]):
    ...
    async def create(self, db: AsyncSession, obj: RegisterUserParam, *, social: bool = False) -> None:
        ...
        if not social:
            ...
            dict_obj.update({'store_id': 0, 'user_type': '00'})
        else:
            dict_obj = obj.model_dump()
    ...
    async def get_list(self, store_id: int, dept: int = None, username: str = None, phone: str = None,
        ...
        if user_id:
            where_list.append(self.model.id == user_id)
        where_list.append(self.model.user_type == '00')
        if where_list:
